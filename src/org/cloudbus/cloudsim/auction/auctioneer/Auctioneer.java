...
public class Auctioneer extends SimEntity{
	 ...
	 */
	private static List<BidCalculation> bidCalcs;
	 ...
	 */
	private static AuctionAllocation allocations = new AuctionAllocation(); 
	/**
	 ...
	 */	
	public static List<BidCalculation> brokerBidCalcs;
	public static List<BidCalculation> datacenterBidCalcs;
	public static List<BidCalculation> maliciousSP=new ArrayList <BidCalculation>();
	
	public static void computeAuction() {

		//Separate broker bids and datacenter bids into two bunches
		brokerBidCalcs = getBidCalcs(bidCalcs, DatacenterBrokerBid.class);
		datacenterBidCalcs = getBidCalcs(bidCalcs, DatacenterBid.class);
		...
		for (BidCalculation brokerBidCalc : brokerBidCalcs) {
			...
			if (brokerVMs != null) { // If broker has request
				...
				for (BidCalculation datacenterBidCalc : datacenterBidCalcs) {
					...
					if (brokerBidCalc.getAverageBid() >= datacenterBidCalc.getAverageBid()) {
						...
						if (datacenterVMs != null) { // If datacenter has offer
							//Log.printLine("Testing, DatacenterBidCalcs"+datacenterBidCalc.getBidderID());
							//Log.printLine("Testing, BrokerBidCalcs"+brokerBidCalc.getBidderID());
							algorithmEngine(brokerBidCalc, brokerVMs, datacenterBidCalc, datacenterVMs);						
						} 
					} 
				 } 
				 ...
				 */				
				satisfactionHandling(brokerBidCalc);
	...
	public static void algorithmEngine(BidCalculation brokerBidCalc, List<Vm> requestedVMs,BidCalculation datacenterBidCalc, List<Vm> offeredVMs) {
		...
		for (Vm requestedVM : requestedVMs) {
			...
			for (Vm offeredVM : offeredVMs) {
				...
				if (VmUtils.canServe(offeredVM, requestedVM)) {
					 ...
					 */     
					     assignVMs(brokerBidCalc, requestedVM, datacenterBidCalc, offeredVM, assignAmount);
					     break inner;
	 ...
	 */
	public static void satisfactionHandling(BidCalculation brokerBidCalc) {
			...
			if (brokerAllocatedVMs != null) {
				for (Vm brokerVM : brokerAllocatedVMs) {
					...
					Set<BidCalculation> allocatedDatacenterBids = 
						allocations.getAllocatedDatacenterBids(brokerVM);
					...
					if (allocatedDatacenterBids != null) {
						for (BidCalculation allocatedDatacenterBid : allocatedDatacenterBids) {
							 ...
							 */
							Vm allocatedDatacenterVM = allocations.getAllocatedVM(brokerVM, allocatedDatacenterBid);
							if (allocatedDatacenterVM != null) {
								int allocatedQuantity = allocations.getAllocatedQuantity(brokerVM, 
										allocatedDatacenterBid);

								/**
								 * Roll back the price and requested amount
								 */
								releaseVMsStep1(brokerBidCalc, brokerVM,allocatedDatacenterBid, allocatedDatacenterVM, allocatedQuantity);
	 ...
	 */
	private static void assignVMs(BidCalculation brokerBidCalc, Vm requestedVM, 
		...
		allocations.addAllocation(requestedVM, datacenterBidCalc, offeredVM, assignAmount, payPrice);
 	}
	 ...
	 * assignment for both sides (broker and datacenter)    
	 */
	private static void releaseVMsStep1(BidCalculation brokerBidCalc, Vm brokerVM, 
			BidCalculation datacenterBidCalc, Vm offeredVM, int assignAmount) {
	 ...
	 * @param brokerVM Vm
	 */
	private static void releaseVMsStep2(Vm brokerVM) {
		allocations.removeAllocation(brokerVM);
	}
    ...
    private static void printBidCalcs(String name, List<BidCalculation> brokerBidCalcs) {
